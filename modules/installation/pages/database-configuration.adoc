...
:description: H5Radar Database configuration: use PostgreSQL for production or H2 for development. Configure DB via application.yml or command line.
:keywords: H5Radar, Axion, database, configuration, PostgreSQL, H2, application.yml, command line, DB setup, credentials, user, password, privileges
...
If you desire to use different configurations fo DB make sure to add them to _application.yml_ or pass them with _-D_ key when launching *Axion* (see xref:application-configuration.adoc[Application configuration]).
